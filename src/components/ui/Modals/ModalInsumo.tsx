...
const ModalInsumo: React.FC<ModalInsumoProps> = ({
  ...
  const [categoria, setCategoria] = useState<number>(
    initialValues.idCategoria || 0
  );
  const [esParaElaborar, setEsParaElaborar] = useState<boolean>(
    initialValues.esParaElaborar || false
  );
  ...
  const validationSchema = Yup.object().shape({
    ...
    stockActual: Yup.number()
      ...
      .min(
        Yup.ref("stockMinimo"),
        "El stock no puede ser menor que el stock mínimo"
      )
      .max(
        Yup.ref("stockMaximo"),
        "El stock no puede ser mayor que el stock máximo"
      ),
    stockMaximo: Yup.number()
      .required("Campo requerido")
      .positive("El stock máximo debe ser un número positivo"),
  ...
  const handleDeleteImg = async (url: string, uuid: string) => {
    ...
    if (insumoImages.length === 1) {
      showModal(
        "Error",
        "No puedes eliminar la última imagen de la empresa",
        "warning"
      );
      return;
    }
  ...
  const handleSubmit = async (values: InsumoPost) => {
    ...
    try {
      ...
      if (isEditMode && insumoAEditar) {
        ...
        response = (await insumoService.postSec(
          `${URL}/ArticuloInsumo`,
          insumoPost,
          token
        )) as IInsumo;
    ...
    >
      <div style={{ display: "flex", flexDirection: "column", gap: "20px", marginTop:'10px'}}>
        ...
        >
          ...
          {isEditMode &&
            insumoAEditar &&
            insumoAEditar?.imagenes.length > 0 && (
              <div>
