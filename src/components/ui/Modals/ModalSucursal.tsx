...
const ModalSucursal: React.FC<ModalSucursalProps> = ({
  ...
  const fetchEmpresa = async () => {
    try {
      ...
      const empre = (await empresaService.getById(
        `${URL}/empresa`,
        idEmpresa,
        token
      )) as IEmpresa;
    ...
    try {
      if (selectedPais) {
        const provincias = await provinciaService.getAll(
          `${URL}/provincia/findByPais/${selectedPais}`
        );
    ...
    try {
      if (selectedProvincia) {
        const localidades = await localidadService.getAll(
          `${URL}/localidad/findByProvincia/${selectedProvincia}`
        );
  ...
  const handleProvinciaChange = (event: ChangeEvent<HTMLSelectElement>) => {
    ...
    const provSeleccionada = provincias.find(
      (provincia) => provincia.nombre === provNombre
    );
    if (provSeleccionada) {
  ...
  const handleLocalidadChange = (event: ChangeEvent<HTMLSelectElement>) => {
    ...
    const localidadSeleccionada = localidades.find(
      (localidad) => localidad.nombre === localidadNombre
    );
  ...
  return (
    <GenericModal
      modalName={modalName}
      title={
        isEditMode ? "Editar Sucursal" : `Añadir Sucursal a ${empresa?.nombre}`
      }
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
        ...
        <div style={{ display: "flex", gap: "20px" }}>
          ...
          <div style={{ flex: 1 }}>
            {selectedPais && (
              <SelectList
                title="Provincias"
                items={provincias.map(
                  (provincia: IProvincia) => provincia.nombre
                )}
          ...
          <div style={{ flex: 1 }}>
            {selectedProvincia && (
              <SelectList
                title="Localidades"
                items={localidades.map(
                  (localidad: ILocalidad) => localidad.nombre
                )}
        ...
        <label style={{ display: "flex", alignItems: "center" }}>
          ...
          <input
            ...
            disabled={
              (isEditMode && !casaMatriz && casaMatrizDisabled) ||
              (!isEditMode && casaMatrizDisabled)
            }
            style={{ marginRight: "10px" }}
          />
        ...
        {(casaMatrizDisabled && !isEditMode) ||
        (casaMatrizDisabled && isEditMode && !casaMatriz) ? (
