...
const ModalPromocion: React.FC<ModalPromocionProps> = ({
  ...
  const handleNewArticle = async (
    ...
    if (idArticulo !== null && cantidad > 0) {
      try {
        ...
        showModal(
          "Error",
          "Ha ocurrido un error al crear el detalle del artículo",
          "error"
        );
      }
    } else {
      ...
      showModal(
        "Advertencia",
        "Debes seleccionar un artículo y especificar una cantidad válida",
        "warning"
      );
    }
  };
  ...
  const columns: Column[] = [
    {
      ...
      label: "",
      renderCell: (element) => (
        <Typography variant="body1">{element.idArticulo}</Typography>
      ),
    },
    {
      id: "cantidad",
      label: "Cantidad",
      renderCell: (element) => (
        <Typography variant="body1">{element.cantidad}</Typography>
      ),
    },
  ];
    ...
    const fetchArticulosPromocion = async () => {
      if (isEditMode && promocionAEditar) {
        try {
          ...
          const detallesPromocion = await promocionDetalleService.get(
            `${URL}/promocion/getDetallesByid`,
            promocionAEditar.id
          );
          const detallesArray = Array.isArray(detallesPromocion)
            ? detallesPromocion
            : [detallesPromocion];
          setDataArticles(detallesArray);
        } catch (error) {
          console.error(
            "Error al cargar los detalles de los artículos:",
            error
          );
        }
      } else {
  ...
  const handleDeleteImg = async (url: string, uuid: string) => {
    ...
    if (promocionImages.length === 1) {
      showModal(
        "Error",
        "No puedes eliminar la última imagen de la promocion",
        "warning"
      );
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        const updatedImages = promocionImages.filter(
          (img) => img.uuid !== uuid
        );
  ...
  const handleTipoPromocionChange = (
    event: SelectChangeEvent<TipoPromocion>
  ) => {
  ...
  const handleSubmit = async (values: PromocionPost) => {
    ...
    try {
      ...
      const promocionPost: PromocionPost = {
        ...
        idSucursales: isEditMode
          ? promocionAEditar.sucursales.map(
              (sucursal: ISucursal) => sucursal.id
            )
          : selectedSucursales,
        ...
        detalles: dataArticles.map((detalle) => {
          return {
            ...
            idArticulo: isEditMode
              ? detalle.insumo !== null
                ? detalle.insumo.id
                : detalle.manufacturado !== null
                ? detalle.manufacturado.id
                : null
              : detalle.idArticulo,
          };
        }),
      ...
      if (isEditMode && promocionAEditar) {
        ...
        response = await promocionService.putSec(
          `${URL}/promocion`,
          promocionAEditar.id,
          promocionPost,
          token
        ...
        response = (await promocionService.postSec(
          `${URL}/promocion`,
          promocionPost,
          token
        )) as IPromocion;
    ...
    >
      <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
        ...
        >
          ...
          {isEditMode &&
            promocionAEditar &&
            promocionAEditar?.imagenes.length > 0 && (
              <div>
