...
const ModalCategoria: React.FC<ModalCategoriaProps> = ({
    ...
    try {
      if (selectedSucursales.length > 0) {
        ...
        if (isEditMode && categoriaAEditar) {
          response = await categoriaService.putSec(
            `${URL}/categoria`,
            categoriaAEditar.id,
            categoriaPost,
            token
          ...
          response = (await categoriaService.postSec(
            `${URL}/categoria`,
            categoriaPost,
            token
          )) as ICategoria;
          ...
          const saveSubCategorias = async (
            ...
            for (const subCategoria of subCategorias) {
              ...
              const subResponse = await fetch(
                `${URL}/categoria/addSubCategoria/${parentId}`,
                {
                  method: "PUT",
                  headers: {
  ...
  const removeSubCategoria = (index: number, parentIndex?: number) => {
    if (parentIndex !== undefined) {
      const newSubCategorias = [...subCategorias];
      newSubCategorias[parentIndex].subCategorias = newSubCategorias[
        parentIndex
      ].subCategorias.filter((_, i) => i !== index);
      setSubCategorias(newSubCategorias);
    } else {
  ...
  const renderSubCategorias = (
    ...
    return subCategorias.map((subCategoria, index) => (
      <div
        key={index}
        style={{
          marginLeft: parentIndex !== undefined ? "20px" : "0px",
          marginTop: "10px",
        }}
      >
        <Stack direction="row" alignItems="center" spacing={6}>
          <TextField
            ...
            onChange={(e) => {
              ...
              if (parentIndex !== undefined) {
                ...
                newParentSubCategorias[parentIndex].subCategorias =
                  newSubCategorias;
          ...
          <Button
            size="small"
            onClick={() => removeSubCategoria(index, parentIndex)}
          >
            Eliminar
          </Button>
        </Stack>
        {subCategoria.subCategorias &&
          renderSubCategorias(subCategoria.subCategorias, index)}
      </div>
    ));
  };
