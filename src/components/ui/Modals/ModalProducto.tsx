...
const ModalProducto: React.FC<ModalProductoProps> = ({
  ...
  const [unidadMedidaProducto, setUnidadMedidaProducto] = useState<number>(
    initialValues.idUnidadMedida || 0
  );
  const [categoriasInsumo, setCategoriasInsumo] = useState<any[]>([]);
  const [selectedCategoria, setSelecteCategoria] = useState<number | null>(
    null
  );
  const [categoriaProductoOptions, setCategoriaProductoOptions] = useState<
    any[]
  >([]);
...
*/
  ...
  const handleDeleteImg = async (url: string, uuid: string) => {
    ...
    if (articuloImages.length === 1) {
      showModal(
        "Error",
        "No puedes eliminar la última imagen del Producto",
        "warning"
      );
      return;
    }
  ...
  const fetchProductoDetalle = async () => {
    if (productoAEditar) {
      try {
        const detalles = await productoDetalleService.getAll(
          `${URL}/ArticuloManufacturado/allDetalles/${productoAEditar.id}`
        );
  ...
  const onDeleteProductoDetalle = async (productoDetalle: IProductoDetalle) => {
    try {
      if (isEditMode && productoAEditar) {
        ...
        const token = await getToken();
        await productoDetalleService.deleteSec(
      ...
      const updatedIngredients = dataIngredients.filter(
        (ingredient) => ingredient.id !== productoDetalle.id
      );
  ...
  const columns: Column[] = [
    ...
    },
    {
      ...
      label: "Cantidad: ",
      renderCell: (element) => <>{element?.cantidad || "N/A"}</>,
    },
  ];
  ...
  const handleSubmit = async (values: IProducto) => {
    ...
    try {
      ...
      if (isEditMode && productoAEditar) {
        ...
        response = (await productoService.postSec(
          `${URL}/ArticuloManufacturado`,
          productoPost,
          token
        )) as IProducto;
