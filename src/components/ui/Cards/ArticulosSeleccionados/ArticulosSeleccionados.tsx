...
const ArticuloSeleccionado: React.FC<ArticuloSeleccionadoProps> = ({
  onAgregarArticulo,
}) => {
  ...
  useEffect(() => {
    const fetchArticulos = async () => {
      try {
        const articulosManufacturados = await articuloService.getAll(
          `${url}/ArticuloManufacturado`
        );
        const articulosInsumo = await insumoService.getAll(
          `${url}/ArticuloInsumo`
        );
        const articulos = articulosInsumo.filter(
    ...
    <>
      ...
      <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
        <FormControl sx={{ minWidth: 200 }}>
          <InputLabel id="select-articulo-label">
