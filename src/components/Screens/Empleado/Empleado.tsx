  ...
  const fetchEmpleados = async () => {
    try {
      const empleados = await empleadoService.getAll(
        `${url}/empleado/bySucursalId/${sucursalId}`
      );
    ...
    try {
      ...
      await onDelete(
        ...
        async (empleadoToDelete: IEmpleado) => {
          await empleadoService.deleteSec(
            `${url}/empleado`,
            empleadoToDelete.id,
            token
          );
  ...
  const columns: Column[] = [
    {
      id: "nombre",
      label: "Nombre",
      renderCell: (rowData) => <>{rowData.nombre}</>,
    },
    {
      id: "apellido",
      label: "Apellido",
      renderCell: (rowData) => <>{rowData.apellido}</>,
    },
    {
      id: "telefono",
      label: "Teléfono",
      renderCell: (rowData) => <>{rowData.telefono}</>,
    },
    {
      id: "email",
      label: "Email",
    ...
    },
    {
      ...
      renderCell: (rowData) => (
        <>
          {new Date(rowData.fechaNacimiento).toLocaleDateString("es-ES", {
            timeZone: "UTC",
          })}
        </>
      ),
