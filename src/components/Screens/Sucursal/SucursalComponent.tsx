  ...
  const fetchSucursal = async () => {
    try {
      ...
      if (empresaId !== undefined) {
        ...
        setEmpresa(empresa);
        const hasCasaMatriz = empresa.sucursales.some(
          (sucursal: ISucursal) => sucursal.esCasaMatriz
        );
    ...
    >
      <Container>
            ...
            {sucursalesEmpresa.length === 0 ? (
              ...
              >
                <EmptyState
                  ...
                  description={`Parece que aún no has creado ninguna sucursal en ${empresa?.nombre}. ¿Te gustaría crear una ahora?`}
                />
              </Box>
                ...
                (sucursal) =>
                  "id" in sucursal && (
                    <Grid item xs={12} sm={6} md={4} key={sucursal.id}>
                      <CardSucursal
                        sucursal={sucursal as SucursalWithId}
                        onEdit={handleEdit}
                        onDelete={onDeleteSucursal}
                      />
                    </Grid>
                  )
              )
        ...
        <ModalSucursal
          modalName="modal"
          initialValues={
            sucursalEditar || generateInitialSucursal(empresa?.id || 0)
          }
